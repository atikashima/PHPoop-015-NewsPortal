<?php
    ...
    if ( Session::exists('tambah_video')){
        echo Session::flash('tambah_video');
    }
    $errors = array();
    ...
    if(Input::get('submit'))
    {
        if( Token::check_token(Input::get('token')) )
        {
            ...
            {
                $admin->tambahvideo(array(
                    'nama_video' => Input::get('judul'),
                    'link_video' => Input::get('link_video'),
                    'isi_news' => Input::get('isi_news'),
                    'date' => date('Y-m-d H:i:s'),
                    'author' => Session::get_session('username')
                    ));
                Session::flash('tambah_video','Berita Berhasil Di Buat');
                Redirect::To('tambahvideo');
            } else {
...
?>
    ...
    <div class="profile">
        <h3 class="title-profile">Tambah Video Berita</h3>
        <form method="post" action="tambahvideo.php">
        Judul Berita : <input type="text" name="judul" style="width:400px;"><br/>
        Link Video Embed : <textarea name="link_video" rows="5" cols="53"></textarea><br/>
        Isi Berita : <textarea name="isi_news" rows="10" cols="100"></textarea><br/>
